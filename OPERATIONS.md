...
- **高**: API key 漏えい、Pi セッションを落とす例外、`/soccer login/logout/status` の破損。
- **中**: `/soccer setup/pick/search` の選択不能、キャッシュ破損、表示データの明確な不整合。
- **低**: 文言改善、対応リーグ追加、表示フォーマット調整、README 改善。

### 受け入れ条件

各 Issue は、最低限以下を明記してから実装します。

- 影響するコマンドまたは表示箇所
- 期待動作と実際の動作
- API key が必要かどうか
- 確認コマンド: 原則 `npm run check`
