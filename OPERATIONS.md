...
| 優先度 | 判断基準 | 例 |
| --- | --- | --- |
| **high（高）** | 秘密情報の漏えい、Pi セッションを落とす不具合、認証フローの破損。放置するとデータ流出やセッション全損につながる。 | API key がログ/Issue に露出、未捕捉例外で Pi が停止、`/soccer:login` `/soccer:logout` `/soccer:status` が機能しない |
| **medium（中）** | 主要コマンドが使えない、データが壊れる/明確に誤る。利用者は回避できるが体験が大きく損なわれる。 | `/soccer:setup` `/soccer:search` で選択不能、キャッシュ破損、表示データの明確な不整合 |
...
- すべての Issue で `npm run ci`（`typecheck` + `test` + `release:check`）の成功を必須条件にします。これは GitHub Actions の CI と同じ検証です。
- `npm run check` は publish ワークフロー用の軽量チェック（`test` + `npm pack --dry-run`）です。Issue 着手時の受け入れ確認には `npm run ci` を使います。
...
`npm run ci` は `release:check` 経由で `npm pack --dry-run` の tarball 内容も検証します。配布物が `extensions/`, `dist/extensions/`, `README.md`, `OPERATIONS.md`, `LICENSE`, `CHANGELOG.md` に収まっているか確認します。
...
- `npm run ci` が通る（TypeScript typecheck、build、テスト、`release:check` を含む）。
- API key 未設定時に案内表示になる。
- API key 設定済み時に `/soccer:status` が key 本体を表示しない。
- `/soccer:search` `/soccer:add` `/soccer:favorite` `/soccer:remove` が UI ピッカーと文字列指定の両方で破綻しない。
